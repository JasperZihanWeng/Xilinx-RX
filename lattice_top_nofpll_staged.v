module top (
    ...
    reg [7:0]  xgmii_txc_0;
    reg [6:0]  tx_fifo_phase;

    // In the documented "without Fabric PLL" clocking mode, the fabric
    // interface runs at 161.1328125 MHz while XGMII carries 64 bits at
    // 156.25 MHz. Write exactly 64 words in every 66 user-clock cycles.
    wire tx_fifo_wr = (tx_fifo_phase < 7'd64);
    ...
    always @(posedge mpcs_tx_out_clk_int_0 or negedge resetn_i) begin
        if (!resetn_i)
            ...
            tx_fifo_phase <= 7'd0;
        else if (tx_fifo_phase == 7'd65)
            tx_fifo_phase <= 7'd0;
        else
            tx_fifo_phase <= tx_fifo_phase + 1'b1;
    end
    ...
    assign mpcs_tx_ch_din_i_0 = {tx_fifo_wr, 6'b0, 1'b0, xgmii_txc_0, xgmii_txd_0};
